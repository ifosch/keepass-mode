...
(defun keepass-utils-read-data-from-string (input)
  "Read data from INPUT string into an alist."
  (mapcar
    (lambda (arg) (split-string arg ":" nil " "))
    (split-string input "\n")))

(defun keepass-utils-get-value-from-alist (key alist)
  "Get the value for KEY from the ALIST."
  (car (cdr (assoc key alist))))

(defun keepass-utils-get-field (field entry)
  "Get FIELD from an ENTRY."
  (keepass-utils-get-value-from-alist field (keepass-utils-read-data-from-string entry)))

(defun keepass-utils-command (group command)
...
(defun keepass-utils-quote-unless-empty (text)
  "Quote TEXT unless it's empty."
  (if (= (length text) 0) text (format "'%s'" text)))

(defun keepass-utils-is-group (entry)
